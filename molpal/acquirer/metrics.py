...
def valid_metrics() -> Set[str]:
    return {
        "random", "threshold", "greedy", "noisy",
        "ucb", "lcb", "ts", "thompson", "ei", "pi",
        "borda",
    }
...
def borda(Y_mean: np.ndarray) -> np.ndarray:
    """Borda count acquisition score.

    Converts predicted means to rank-based scores so that the acquisition
    is robust to scale differences across surrogate backbones.  Molecules
    are scored by their rank (highest mean → highest score), making this
    equivalent to greedy on the rank rather than the raw prediction.

    This is the natural companion to BigFusionSurrogate and
    EnsembleFusionSurrogate, which internally combine Borda counts across
    backbones and return a fused rank score as mu.

    Parameters
    ----------
    Y_mean : np.ndarray
        Mean predicted values from the surrogate.

    Returns
    -------
    np.ndarray
        Borda acquisition scores (higher is better).
    """
    n = len(Y_mean)
    # argsort ascending → rank array: rank[i] = position of molecule i in sorted order
