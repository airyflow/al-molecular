...
class MoleculeDataset(Dataset):
    ...
    def batch_graph(self) -> List[BatchMolGraph]:
        ...
        if self._batch_graph is None:
            ...
            # NOTE: a per-molecule global SMILES_TO_GRAPH cache was tried here
            # (chemprop declares the dict and cache_graph()/set_cache_graph()
            # but never actually wires them into batch_graph() in this vendored
            # version) and reverted -- measured directly at 153.4 KB/molecule,
            # which extrapolates to ~15.3TB for AmpC's 99.5M-molecule pool.
            # BatchMolGraph is a dense, feature-heavy structure, not a cheap
            # object; caching it per-molecule at this pool scale is a much
            # bigger memory problem than the CPU time it would save. Left
            # un-cached, matching the original (safe) behavior.
...
class MoleculeDataLoader(DataLoader):
    ...
    def __init__(
        ...
        self._context = None
        self._timeout = 0
        # Originally gated on "not is_main_thread" -- but predict()/train() call
        # model.to(device) (initializing CUDA in this process) before ever
        # constructing a MoleculeDataLoader, regardless of which thread that
        # happens on. Forking worker processes (the default "fork" start
        # method) from a process with an already-initialized CUDA context is
        # a well-documented deadlock/crash pattern; "forkserver" avoids it by
        # forking from a clean server process instead. Always use it when
        # num_workers > 0, not just off the main thread.
