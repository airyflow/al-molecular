...
class LitMolCLR(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self._train_losses.append(output.detach())
        return output

    def on_train_epoch_end(self):
        if self._train_losses:
            train_loss = torch.stack(self._train_losses).mean()
            if self.logger:
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        self._val_outputs.append(output.detach())
        return output

    def on_validation_epoch_end(self):
        if self._val_outputs:
            val_loss = torch.cat(self._val_outputs).mean()
            if self.logger:
