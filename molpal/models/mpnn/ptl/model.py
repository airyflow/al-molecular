...
class LitMPNN(pl.LightningModule):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        # PL >=2.0 removed training_epoch_end/validation_epoch_end (which received
        # accumulated step outputs as an argument) in favor of on_*_epoch_end (no
        # argument) -- outputs must now be accumulated manually as instance state.
    ...
    def training_step(self, batch: Tuple, batch_idx) -> torch.Tensor:
        ...
        self._train_step_losses.append(output.detach())
        return output

    def on_train_epoch_end(self):
        train_loss = torch.stack(self._train_step_losses, dim=0).mean()
    ...
    def validation_step(self, batch: Tuple, batch_idx) -> List[float]:
        ...
        self._val_step_losses.append(output)
        return output

    def on_validation_epoch_end(self):
        val_loss = torch.cat(self._val_step_losses).mean()
        """
    ...
    def num_training_steps(self) -> int:
        ...
        # trainer.num_gpus/num_processes/tpu_cores were removed in PL >=2.0,
        # unified into trainer.num_devices across all accelerator types.
