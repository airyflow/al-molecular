...
# NOTE: joblib/ray are only used by LightGBMModel below (unused in this
# repo's actual pipeline -- run_experiment.py's --model choices are
# mpn/rf/nn only). RFModel no longer uses either; see its train()/
# get_means()/get_means_and_vars() for why the Ray indirection was removed
# there specifically (a real, reproduced bug, not just unnecessary
# complexity). register_ray() (needed for LightGBMModel's joblib backend)
# is intentionally not called here since nothing currently exercises that
# path -- add it back if LightGBMModel is ever actually used.
...
class RFModel(Model):
    ...
    def train(
        ...
        *,
        ...
        # RandomForestRegressor(n_jobs=-1) already parallelizes internally via
        # sklearn's own default joblib backend (loky) -- no Ray involved. This
        # used to run inside `with joblib.parallel_backend("ray"):`, routing
        # through a Ray actor pool instead; that pool wasn't being cleanly
        # torn down between AL rounds (each round opens a fresh
        # parallel_backend("ray") context), and accumulated across rounds
        # until a worker died with a connection error on round 2 (confirmed
        # directly: round 1 succeeded, round 2 crashed, on an exclusive full-
        # memory node, ruling out a plain memory-cap explanation). Removing
        # the Ray indirection removes the whole class of failure, not just
        # this instance of it.
        self.model.fit(X, Y)
    ...
    def get_means_and_vars(self, xs: Sequence) -> Tuple[np.ndarray, np.ndarray]:
        X = np.vstack(xs)
        # Per-tree predictions for variance, computed directly in-process
        # instead of via ray.put/ray.get + remote per-tree calls -- same
        # rationale as train()/get_means() above. self.model.n_jobs (-1) already
        # governs how the ensemble itself parallelizes; per-tree prediction
        # here is comparatively cheap and not worth a separate Ray/joblib
        # indirection.
        predss = np.array([tree.predict(X) for tree in self.model.estimators_])
